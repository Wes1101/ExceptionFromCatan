...
public class CatanBoard {
  ...
  public void generateHexSpiral(int n, Node[] nodes) {
    ...
    // Zentrum
    this.hexTiles[0] =
      new HexTile(0, 0, 0, "wood", getNodesForHex(0, 0, nodes));
    this.hexTiles[1] =
      new HexTile(
        directionMap.get("SW")[0],
        directionMap.get("SW")[1],
        0,
        "wood",
        getNodesForHex(
          directionMap.get("SW")[0],
    ...
    this.hexTiles[2] =
      new HexTile(
        directionMap.get("SE")[0],
        directionMap.get("SE")[1],
        0,
        "wood",
        getNodesForHex(
          directionMap.get("SE")[0],
    ...
    this.hexTiles[3] =
      new HexTile(
        directionMap.get("E")[0],
        directionMap.get("E")[1],
        0,
        "wood",
        getNodesForHex(
          directionMap.get("E")[0],
    ...
    this.hexTiles[4] =
      new HexTile(
        directionMap.get("NE")[0],
        directionMap.get("NE")[1],
        0,
        "wood",
        getNodesForHex(
          directionMap.get("NE")[0],
    ...
    this.hexTiles[5] =
      new HexTile(
        directionMap.get("NW")[0],
        directionMap.get("NW")[1],
        0,
        "wood",
        getNodesForHex(
          directionMap.get("NW")[0],
    ...
    this.hexTiles[6] =
      new HexTile(
        directionMap.get("W")[0],
        directionMap.get("W")[1],
        0,
        "wood",
        getNodesForHex(
          directionMap.get("W")[0],
    ...
    while (index < n) {
      ...
      for (String dir : directions) {
        for (int step = 0; step <= layer; step++) {
          ...
          this.hexTiles[index] =
            new HexTile(q, r, 0, "wood", getNodesForHex(q, r, nodes));
          index++;
  ...
  public static int calculateNodeId(int q, int r, int i) {
    ...
    int offsetInRing;
    if (r == d) offsetInRing = d - q; else if (q + r == d) offsetInRing =
      2 * d + q; else if (q == d) offsetInRing = 4 * d - r; else if (
      r == -d
    ) offsetInRing = 5 * d - q; else offsetInRing = 3 * d - r;
